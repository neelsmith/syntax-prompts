...
Now create a graph in Mermaid format based on the table of tokens. The mermaid graph type will be `BT`. Whenever we represent a row with a node in the graph, use the "Token" column as the label for the node.
